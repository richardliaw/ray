...
DEFAULT_CONFIG = {
    ...
    # Arguments to pass to the AsyncSamplesOptimizer
    "optimizer": {
        # Number of gradients applied for each `train` step
        "learning_starts": 0,
        "num_replay_buffer_shards": 0
    ...
    def _init(self):
        ...
        self.remote_evaluators = [
            remote_cls.remote(
                ...
                batch_steps=self.config["batch_size"],
                batch_mode="complete_episodes", sample_async=True,
                tf_session_creator=session_creator,
                env_config=self.config["env_config"],
                model_config=self.config["model"], policy_config=self.config,
                num_envs=self.config["num_envs"],
...
if __name__ == '__main__':
    ...
    agent = ImpalaAgent(env="CartPole-v0", config=config)
    for i in range(20):
